...
var LOGGER = require(PATH.join(__dirname, 'module.logger.js')),
	...
	APP = require(PATH.join(__dirname, 'app.fileserver.js'));
...
if(CONFIG.server.ssl && CONFIG.server.ssl.crt && CONFIG.server.ssl.ca && CONFIG.server.ssl.key){ //выбираю тип сервера
	...
	var server = HTTPS.createServer(ssl, APP).setTimeout(timeout).listen(CONFIG.server.streamport, '0.0.0.0');
	LOGGER.log('https-сервер запущен на порту:' + CONFIG.server.streamport);
} else { 
	var server = HTTP.createServer(APP).setTimeout(timeout).listen(CONFIG.server.streamport, '0.0.0.0');
	LOGGER.log('http-сервер запущен на порту:' + CONFIG.server.streamport);
}